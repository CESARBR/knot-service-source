...
static void on_node_channel_data_error(void *user_data)
{
	struct session *session = user_data;
	...
	struct session *session)
{
	...
	l_io_set_read_handler(channel, on_node_channel_data, session,
		on_node_channel_data_error);
	l_io_set_disconnect_handler(channel, on_node_channel_disconnected,
		session, on_node_channel_destroyed);
...
{
	l_queue_destroy(session_list, (l_queue_destroy_func_t) session_destroy);
}
